...
public class MailServer extends Thread {
    ...
    public MailServer(JTextArea taLog) {
        this.taLog = taLog; users = new DefaultListModel<>(); loadStaticData();
    }
    ...
    public void run() {
        try {
            serverSocket = new ServerSocket(Main.PORT_POP3); running = true;
            taLog.append("Mail-Server gestartet auf " + SocketUtils.getHostIP() + " und Port " + Main.PORT_POP3 + "\n");
            while (running) {
                Socket clientSocket = serverSocket.accept(); MailClient client = new MailClient(this, clientSocket);
                client.start();
            }
        } catch (IOException e) {
            taLog.append("FEHLER im MailServer"); e.printStackTrace();
        }
    }

    public void stopThread() {
        running = false; taLog.append("POP3-Server gestoppt.\n");
    }
    ...
    private void loadStaticData() {
        ...
        bob.getMails().add(0, new Mail("sokrates", localize("bob"), "Spam!", "Hi!", new Date())); users.add(1, bob);
    }
}
